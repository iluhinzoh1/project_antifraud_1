...
/** Класс для обработки поступающих топиков из проекта transfer */
...
public class TransferConsumer {
    ...
    private static final String UNKNOWN = "Неизвестный тип перевода: ";
    ...
    public void listenAccount(@Payload TransferChecked transfer) {
        if (isBlocked(transfer.getAmount())) {
            final boolean exists = accountService.existsByTransferId(transfer.getAccountDetailsId());
            if (exists) {
        ...
        } else if (!isBlocked(transfer.getAmount()) && accountService.
                existsByTransferId(transfer.getAccountDetailsId())) {
            cleanUpdate(ACCOUNT, transfer, transfer.getId());
            verdictProducer.sendVerdict(transfer.getId(), ALLOWED, transfer.getPurpose());
        } else {
            cleanCreate(ACCOUNT, transfer);
        }
    }
    ...
    public void listenCard(@Payload TransferChecked transfer) {
        if (isBlocked(transfer.getAmount())) {
            final boolean exists = cardService.existsByTransferId(transfer.getAccountDetailsId());
            if (exists) {
            ...
            verdictProducer.sendVerdict(transfer.getId(), BLOCKED, transfer.getPurpose());
        } else if (!isBlocked(transfer.getAmount()) && cardService.
                existsByTransferId(transfer.getAccountDetailsId())) {
            cleanUpdate(CARD, transfer, transfer.getId());
            verdictProducer.sendVerdict(transfer.getId(), ALLOWED, transfer.getPurpose());
        } else {
            cleanCreate(CARD, transfer);
        }
    }
    ...
    public void listenPhone(@Payload TransferChecked transfer) {
        if (isBlocked(transfer.getAmount())) {
            final boolean exists = phoneService.existsByTransferId(transfer.getAccountDetailsId());
            if (exists) {
            ...
            verdictProducer.sendVerdict(transfer.getId(), BLOCKED, transfer.getPurpose());
        } else if (!isBlocked(transfer.getAmount()) && phoneService.
                existsByTransferId(transfer.getAccountDetailsId())) {
            cleanUpdate(PHONE, transfer, transfer.getId());
            verdictProducer.sendVerdict(transfer.getId(), ALLOWED, transfer.getPurpose());
        } else {
            cleanCreate(PHONE, transfer);
        }
    }
